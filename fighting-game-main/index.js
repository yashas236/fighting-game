...
const gravity = 0.7;
const maxJumpHeight = 50; // Maximum height above ground level the player can jump
const maxXPosition = canvas.width - 50; // Right boundary for characters
const minXPosition = 0; // Left boundary for characters
...
function animate() {
  ...
  // Prevent players from jumping out of bounds
  if (player.position.y <= maxJumpHeight) {
    player.velocity.y = Math.max(player.velocity.y, 0);
  }
  if (enemy.position.y <= maxJumpHeight) {
    enemy.velocity.y = Math.max(enemy.velocity.y, 0);
  }

  // Prevent players from moving out of bounds on the x-axis
  player.position.x = Math.min(Math.max(player.position.x, minXPosition), maxXPosition);
  enemy.position.x = Math.min(Math.max(enemy.position.x, minXPosition), maxXPosition);
  ...
  if (!player.dead) {
    switch (event.key) {
      ...
      case 'a':
        ...
        break;
      case 'w':
        if (player.position.y >= maxJumpHeight) player.velocity.y = -20;
        break;
      case ' ':
        player.attack();
        break;
  ...
  if (!enemy.dead) {
    switch (event.key) {
      ...
      case 'ArrowLeft':
        ...
        break;
      case 'ArrowUp':
        if (enemy.position.y >= maxJumpHeight) enemy.velocity.y = -20;
        break;
      case 'ArrowDown':
        enemy.attack();
        break;
